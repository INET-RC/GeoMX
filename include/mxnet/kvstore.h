...
class KVStore {
 public:
  ...
  /**
   * \return whether or not this process is a global server node.
   *
   * Always returns false when type == "local"
   */
  static bool IsGlobalServerNode() {
#if MXNET_USE_DIST_KVSTORE
    const char* role_str = ps::Environment::Get()->find("DMLC_ROLE_GLOBAL");
    return (role_str != nullptr) && (!strcmp(role_str, "global_server"));
#else
    return false;
#endif  // MXNET_USE_DIST_KVSTORE
  }
  ...
  /**
   * \return whether or not this process is a global scheduler node.
   *
   * Always returns false when type == "local"
   */
  static bool IsGlobalSchedulerNode() {
#if MXNET_USE_DIST_KVSTORE
    const char* role_str = ps::Environment::Get()->find("DMLC_ROLE_GLOBAL");
    return (role_str != nullptr) && (!strcmp(role_str, "global_scheduler"));
#else
    return false;
#endif  // MXNET_USE_DIST_KVSTORE
  }
  ...
  /**
  * \return whether or not this process is a master worker node.
  *
  * Always returns false when type == "local"
  */
  static bool IsMasterWorkerNode() {
#if MXNET_USE_DIST_KVSTORE
    return ps::IsMasterWorker();
#else
    return false;
#endif  // MXNET_USE_DIST_KVSTORE
  }
  ...
  /*!
   * \return The number of worker nodes in global
   */
   virtual int get_all_worker_size() const {
    return 1;
   }
  ...
  * Always returns false when type == "local"
  */
   virtual bool is_master_worker() const {
     return false;
   }
   ...
   */
  virtual void Barrier(const bool is_global = false) { }
