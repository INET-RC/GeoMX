...
/** \brief return The number of all workers in global
 * \param handle handle to the KVStore
 * \param ret the worker size in global
 * \return 0 when success, -1 when failure happens
 */
MXNET_DLL int MXKVStoreGetAllWorkerSize(KVStoreHandle handle,
                                           int *ret);
...
/**
 * \brief return whether or not this process is a global server node.
 * \param ret 1 for yes, 0 for no
 * \return 0 when success, -1 when failure happens
 */
MXNET_DLL int MXKVStoreIsGlobalServerNode(int *ret);

/**
 * \brief return whether or not this process is a global scheduler node.
 * \param ret 1 for yes, 0 for no
 * \return 0 when success, -1 when failure happens
 */
MXNET_DLL int MXKVStoreIsGlobalSchedulerNode(int *ret);

/**
 * \brief return whether or not this process is a master worker node.
 * \param handle handle to the KVStore
 * \param ret 1 for yes, 0 for no
 * \return 0 when success, -1 when failure happens
 */
MXNET_DLL int MXKVStoreIsMasterWorkerNode(KVStoreHandle handle,
                                          int *ret);
