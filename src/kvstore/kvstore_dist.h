...
class KVStoreDist : public KVStoreLocal {
 public:
  explicit KVStoreDist(bool use_device_comm)
    ...
    if (IsWorkerNode() && !IsGlobalSchedulerNode()) {
  ...
  virtual ~KVStoreDist() {
    ...
    if (IsWorkerNode() && !IsGlobalSchedulerNode()) {
      if (barrier_before_exit_) {
        Barrier();
        if (!ps::IsMasterWorker() && get_rank() == 0 &&
              ps_worker_->get_customer()->customer_id() == 0) {
  ...
  // Merge data from other worker nodes.
  void WorkersMerge(const ps::KVMeta& req_meta, const ps::KVPairs<char> &req_data, ps::KVWorker<char>* worker) {
    ...
    if(req_meta.num_merge == -1) {
      ...
      // Send the data.
      ps_worker_->Send(req_meta_buf[key].timestamp, req_meta_buf[key].push, req_meta_buf[key].cmd, req_data_buf[key], req_meta_buf[key].key , req_meta_buf[key].version, req_meta_buf[key].app_id, req_meta_buf[key].customer_id,req_meta_buf[key].num_merge);
  ...
  void CommandHandle(const ps::SimpleData& recved, ps::SimpleApp* app) {
    ...
    if (recved_type == CommandType::kSetGradientCompression) {
  ...
  int get_all_worker_size() const override { return ps::NumAllWorkers(); }

  int get_rank() const override { return ps::MyRank(); }

  bool is_master_worker() const override { return ps::IsMasterWorker(); }
  ...
  void RunServer(const Controller& controller) override {
    CHECK(!IsWorkerNode() || IsGlobalSchedulerNode());
    if (IsServerNode()) {
