...
KVStore* KVStore::Create(const char *type_name) {
  ...
  if (has("dist")) {
    ...
    if (kv->IsWorkerNode() && !kv->IsGlobalSchedulerNode() && kv->get_rank() == 0) {
      // configure the servers to be synchronous mode
      kv->SendCommandToServers(static_cast<int>(kvstore::CommandType::kSyncMode), "");
    }
    if (has("_sync") && kv->IsMasterWorkerNode()) {
      // configure the global server to be synchronous mode
      kv->SendCommandToServers(static_cast<int>(kvstore::CommandType::kSyncGlobalMode), "");
    }
#else
