...
class KVStore(object):
    ...
    @property
    def num_all_workers(self):
        """Returns the number of all worker nodes in global.

        Returns
        -------
        size :int
            The number of all worker nodes in global.
        """
        size = ctypes.c_int()
        check_call(_LIB.MXKVStoreGetAllWorkerSize(self.handle, ctypes.byref(size)))
        return size.value

    @property
    def is_master_worker(self):
        """Returns whether this node is a master worker node.

        Returns
        -------
        is_master :bool
            true if this node is a master worker else false
        """
        is_master = ctypes.c_int()
        check_call(_LIB.MXKVStoreIsMasterWorkerNode(self.handle, ctypes.byref(is_master)))
        return bool(is_master.value)
