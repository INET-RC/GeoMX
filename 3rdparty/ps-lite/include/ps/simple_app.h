...
class SimpleApp {
 public:
   ...
   */
  virtual inline int Request(int req_head, const std::string& req_body, int recv_id, bool is_global = false);
...
inline int SimpleApp::Request(int req_head, const std::string& req_body, int recv_id, bool is_global) {
  // setup message
  Message msg;
  ...
  // send
  for (int r : Postoffice::Get()->GetNodeIDs(recv_id, is_global)) {
    msg.meta.recver = r;
    Postoffice::Get()->van()->Send(msg, is_global);
  }
  return ts;
}
...
inline void SimpleApp::Response(const SimpleData& req, const std::string& res_body) {
  ...
  // send
  Postoffice::Get()->van()->Send(msg, req.sender < ps::kOffset);
}
